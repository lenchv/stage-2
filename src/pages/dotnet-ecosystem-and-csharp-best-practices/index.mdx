...
<ul>
  ...
  <li class="details">
    ...
    <div class="details-body" style="margin-bottom: 0.5em;">
      ...
      <p>У .NET-і очищення пам'яті відбувається автоматично. За це відповідає <strong>Garbage Collector</strong> (по нашому — зборщик мусору). Коли він бачить, що на об'єкт в кучі більше немає посилань, він видаляє цей об'єкт та очищує пам'ять.</p>
    </div>
  </li>
  <li class="details">
...
<ul>
  ...
  <li class="details">
    ...
    <div class="details-body" style="margin-bottom: 0.5em;">
      <p>Використовуючи ключове слово <code>params</code>, можна вказувати параметру методу, що він приймає довільну кількість аргументів — нуль або більше. Далі ці аргументи в тілі методу можна використовувати у контексті масиву.</p>
      <SnippetL03S02 />
      <p>Передавати аргументи у метод, що має змінну кількість параметрів, можна просто перераховуючи їх через кому, як показано на прикладі. Варто зазначити, що аргумент <code>params</code> повинен бути вказаним останнім після усіх строго визначених аргументів методу.</p>
    </div>
  </li>
  <li class="details">
    ...
    <div class="details-body" style="margin-bottom: 0.5em;">
      ...
      Це означає, що ми зосерджуємось на тому, що об'єкт може робити, а не на тому, як він це робить. Це часто використовується у написанні великих і складних програм. 
      Щоб цього досягти використовуються <em>абстрактні класи</em> та <em>інтерфейси</em>.</p>
      <p>У <em>абстрактном класі</em> ми можемо <strong>створити функціональність, яка реалізується у похідному класі</strong>. 
      ...
      <p>Клас імплеменує інтерфейс та обов'язково реалізує ці методи.
      ...
      <ol>
        ...
        <li>
          В <em>Інтерфейсі</em> ми можемо лише описати сигнатуру методу без його імплементації. У <em>абстрактному класі</em> ми можемо повністю імплементувати метод, який нас цікавить. Для цього метод має бути не абстрактним.          
          <div class="columns">
            <div class="column col-6">
              <SnippetL03S05 />
            </div>
        ...
        <li>
          <em>Інтерфейс</em> не може містити полів — лише властивості, <em>абстрактний клас</em> — може.
          <div class="columns">
            <div class="column col-6">
              <SnippetL03S10 />
            </div>
      ...
      <p>Інтерфейс ми використовуємо щоб описати АРІ для кількох класів, які, швидше за все, будуть імплементувати більше одного інтерфейсу. Треба пам'ятати, що члени інтерфейсу не можуть бути статичними. C#, на відміну від С++, не підтримує множинне наслідування, тому щоб це реалізувати, ми використовуємо інтерфейси.</p>
  ...
  <li class="details">
    ...
    <div class="details-body" style="margin-bottom: 0.5em;">
      ...
      <p>У C# є <em>Constraint</em>-и для того щоб обмежити типи, які можна використовувати у дженерік класі. Наприклад, якщо <em>Constraint</em>-ом ми вказуємо що типом <code>Т</code> може бути тільки <em>reference type</em>, тобто класи, то ми не зможемо використати <em>value type</em>-и для створення екземпляру дженерік класу.
  ...
  <li class="details">
    ...
    <div class="details-body" style="margin-bottom: 0.5em;">
      <p>Явно <em>структурному типу даних</em> ми не можемо присвоїти <code>null</code>. Щоб це зробити, нам потрібно оголосити змінну з модифікатором <code>?</code>. Цей модифікатором являється аліасом до структури <code>Nullable&lt;T&gt;</code>, що є так званою обгорткою для структур, що і показано на сигнатурі.</p>
      <SnippetL03S18 />
  ...
  <li class="details">
    ...
    <div class="details-body" style="margin-bottom: 0.5em;">
      ...
      <p><em>Делегати</em> — це об'єкти, які вказують на методи. За допомогою них ми можемо викликати дані методи.</p>
      <SnippetL03S21 />
      <p>Спочатку ми оголошуємо <em>сигнатуру делегату</em>. Це означає, що методи, на які може вказувати делегат, мають мати такі ж самі параметри та повертаючий тип, що і делегат.</p>
      <p>У нашому випадку делегат <code>LogMessage</code> має один параметр <code>string</code> і нічого не повертає.</p>
      <p>В класі програм ми оголосили два методи: <code>LogRedMessage</code> та <code>LogGreenMessage</code> з тою самою сигнатурою, що і у делегата, тому ми можемо присвоїти йому посилання на ці методи.</p>
