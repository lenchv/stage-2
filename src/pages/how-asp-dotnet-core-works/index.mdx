...
<h4>What you need to start</h4>
You need to have a PC/laptop with Windows/Linux/macOS system installed.  
Microsoft provides two options — .NET Runtime and .NET SDK:
...
* **.NET Core SDK** includes everything you need to build and run .NET Core applications, using command line tools and any editor (including Visual Studio).
  
  <div class="bubble small subtle">
    Для выполнения домашки вам нужен <strong>.NET Core SDK</strong>.
  </div>
...
<div class="bubble small subtle">
  Я б рекомендовал использовать <strong>Visual Studio Code</strong> для выполнения домашки.
</div>
...
<h4>Create a new app by running <code>dotnet new webapi</code> <sup><strong><a href="https://docs.microsoft.com/en-us/aspnet/core/fundamentals/startup?view=aspnetcore-2.2">[1]</a></strong></sup></h4>
<h6>Program.cs</h6>
...
You should receive `["value1","value2"]` JSON as a response.

<div class="bubble small subtle">
  You can use Postman for testing your API. But I would recommend using a VSCode extension called <a href="https://marketplace.visualstudio.com/items?itemName=humao.rest-client"><strong>REST Client</strong></a>.
</div>
